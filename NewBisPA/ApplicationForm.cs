    ...
    public partial class ApplicationForm : Form
    {
        ...
        private void btnHeadClear_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                RiskDesc.Text = "ระดับความเสี่ยง";
                RiskDesc.ForeColor = Color.Black;
        ...
        private void txtAppNo_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyValue == 13)
            {
                try
                {
                    ...
                    //ตรวจสอบข้อมูล Amlo  Dev.Mac
                    #region "ตรวจสอบ RiskCode"
                    var baseUrlAmlo = ITUtility.Utility.AppSettings("AmloNewbisAPI");
                    var amloApi = new RestClient(baseUrlAmlo);


                    var checkAmloDataRequest = new RestRequest("AmloNewbis/CheckAmloData/{appNo}", Method.GET);
                    checkAmloDataRequest.AddParameter("appNo", txtAppNo.Text, ParameterType.UrlSegment);
                    var chkAmloResponse = amloApi.Execute(checkAmloDataRequest);
                    CheckAmloData checkAmlodata = null;
                    if (chkAmloResponse.StatusCode == System.Net.HttpStatusCode.OK)
                    {
                        checkAmlodata = JsonConvert.DeserializeObject<CheckAmloData>(chkAmloResponse.Content);
                        if (checkAmlodata != null)
                        {
                            if (checkAmlodata.Status.Equals("Y"))
                            {
                                RiskDesc.Text = checkAmlodata.RiskDesc;
                                RiskDesc.ForeColor = Color.Red;
                            }
                        }
                    }

                    #endregion"ตรวจสอบ RiskCode"
        ...
        private void GetAmloDataLink()
        {
            var baseUrlAmlo = ITUtility.Utility.AppSettings("AmloNewbisAPI");
            var amloApi = new RestClient(baseUrlAmlo);

            var getInfoRequest = new RestRequest("AmloNewbis/GetInfoAppNo", Method.GET);
            getInfoRequest.AddParameter("appNo", txtAppNo.Text);
            var getInforRespons = amloApi.Execute(getInfoRequest);
            AppNoInfo appNoInfo = null;
            if (getInforRespons.StatusCode == System.Net.HttpStatusCode.OK)
            {
                appNoInfo = JsonConvert.DeserializeObject<AppNoInfo>(getInforRespons.Content);
            }
            if (appNoInfo != null)
            {
                var restRequet = new RestRequest("AmloNewbis/GetAmloDataLink", Method.POST);
                restRequet.RequestFormat = DataFormat.Json;
                restRequet.AddJsonBody(new
                {
                    RiskInfo = appNoInfo.RiskInfos,
                    PlanId = appNoInfo.PlanCode,
                    PlanDesc = appNoInfo.PlanDesc,
                    Policy = appNoInfo.Policy,
                    PolicyId = appNoInfo.PolicyId,
                    AppNo = appNoInfo.AppNo,
                    UserId = _UserID
                });
                var response = amloApi.Execute(restRequet);
                AmloDataLink amloDataLink = null;
                if (response.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    amloDataLink = JsonConvert.DeserializeObject<AmloDataLink>(response.Content);

                    if (amloDataLink != null)
                    {
                        if (Convert.ToInt32(amloDataLink.RiskCode) > 1)
                        {
                            System.Diagnostics.Process.Start("chrome.exe", amloDataLink.Url);
                        }
                        else if (Convert.ToInt32(amloDataLink.RiskCode) <= 1)
                        {
                            RiskDesc.Text = amloDataLink.RiskDesc;
                            RiskDesc.ForeColor = Color.Red;
                        }
                    }
                }
                ...
                if (cmbSummryStatus.SelectedValue != null)
                {
                    ...
                    if (cmbSummryStatus.SelectedValue.ToString() == "IF")
                    {
                        ...
                        if (chkAmloResponse.StatusCode == System.Net.HttpStatusCode.OK)
                        {
                            ...
                            if (checkAmlodata != null)
                            {
                                if (checkAmlodata.Status.Equals("N"))
                                {
                                    throw new Exception("ไม่สามารถออกกรมธรรม์ได้ เนื่องจากยังไม่ได้ตรวจสอบข้อมูล Amlo");
                                }
                                if (checkAmlodata.RiskCode.Equals("4"))
                                {
                                    throw new Exception("ไม่สามารถออกกรรธรรม์ได้ เนื่องจากสถานะการตรวจสอบ Amlo คือ : " + checkAmlodata.RiskDesc);
                                }
                            }
                        }
            ...
            try
            {
                if (e.KeyValue == 13)
                {
                    if (txtBranchCode.Text.Trim() != "")
                    {
                        ...
                        {
                            txtAccName.Text = $"{txtPrename.Text}{txtName.Text} {txtSurname.Text}";
                        }
        ...
        private void BtnSearchPlanPaid_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (planSearchForm.PlanResult != null)
                {
                    ...
                    var ulifePlan = PAR_PA_APPLICATION_ID?.APPLICATION_Collection?.Where(item => item.LIFE_ID != null && item.LIFE_ID.UAPP_ID != null).FirstOrDefault();
                    if (PAR_PA_APPLICATION_ID.APP_ID == null || ulifePlan == null)
                    {
                        if (channelType == "KB")
                        {
                            int cusAge = txtStAge.Text.Trim() == "" ? 0 : Convert.ToInt32(txtStAge.Text.Trim());
        ...
        private void btnChkAmlo_Click(object sender, EventArgs e)
        {
            GetAmloDataLink();
        }
    }
