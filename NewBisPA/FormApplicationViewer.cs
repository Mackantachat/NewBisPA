...
namespace NewBisPA
{
    public partial class FormApplicationViewer : Form
    {
        ...
        public FormApplicationViewer() {
            InitializeComponent();
        }

        public FormApplicationViewer(string _user_id)
        {
            InitializeComponent();
            UserID = _user_id;
        }

        private void FormApplicationViewer_Load(object sender, EventArgs e)
        {
        ...
        private void bindingControl()
        {
            using (NewBISSvcRef.NewBISSvcClient client = new NewBISSvcRef.NewBISSvcClient())
            {
                try
                {
                    ...
                    //โอนสิทธิ์เงินจ่ายตามเงื่อนไข 
                    //DataTable assignment = new DataTable();
                    //assignment.Columns.Add(new DataColumn("Display", typeof(string)));
                    //assignment.Columns.Add(new DataColumn("Id", typeof(string)));
                    //assignment.Rows.Add(assignment.NewRow());
                    //assignment.Rows.Add(assignment.NewRow());
                    //assignment.Rows.Add(assignment.NewRow());
                    //assignment.Rows[0][0] = "ผ่าน";
                    //assignment.Rows[0][1] = "Y";
                    //assignment.Rows[1][0] = "ไม่ผ่าน";
                    //assignment.Rows[1][1] = "N";
                    //assignment.Rows[2][0] = "ทั้งหมด";
                    //assignment.Rows[2][1] = "ALL";

                    //cmbassignment.DataSource = assignment;
                    //cmbassignment.DisplayMember = "Display";
                    //cmbassignment.ValueMember = "Id";
                    //cmbassignment.SelectedValue = "ALL";

                    //cmbprocflg
                    DataTable processflg = new DataTable();
                    processflg.Columns.Add(new DataColumn("Display", typeof(string)));
                    processflg.Columns.Add(new DataColumn("Id", typeof(string)));
                    processflg.Rows.Add(processflg.NewRow());
                    processflg.Rows.Add(processflg.NewRow());
                    processflg.Rows.Add(processflg.NewRow());
                    processflg.Rows[0][0] = "จบขั้นตอน";
                    processflg.Rows[0][1] = "Y";
                    processflg.Rows[1][0] = "ยังไม่จบขั้นตอน";
                    processflg.Rows[1][1] = "N";
                    processflg.Rows[2][0] = "ทั้งหมด";
                    processflg.Rows[2][1] = "";

                    cmbprocflg.DataSource = processflg;
                    cmbprocflg.DisplayMember = "Display";
                    cmbprocflg.ValueMember = "Id";
                    cmbprocflg.SelectedValue = "";
        ...
        private void chknotund_CheckedChanged(object sender, EventArgs e)
        {
        ...
        {
            if (e.RowIndex < 0 || e.ColumnIndex != dtgCallReport.Columns["clmlinkappno"].Index)
            {
                return;
            }
            ...
            { 
                    ...
                    //mainForm.APPID = Convert.ToInt64(dtgCallReport["clmappid", e.RowIndex].Value);
                    //mainForm.FORMAPP = "APPVIEWER";
                ...
                FormApplication.SelectAppFromMenu("VIEW", null, this, appno, channel);//("APPVIEWER", Convert.ToInt64(dtgCallReport["clmappid", e.RowIndex].Value), this);
        ...
        { 
            ...
            {
                MessageBox.Show("เงินในบัญชีเริ่มต้นต้องมีค่าน้อยกว่า");
                txtsumsource.Focus();
            }
            ...
            using (NewBISSvcRef.NewBISSvcClient client = new NewBISSvcRef.NewBISSvcClient())
            {
                ...
                try
                {
                    ...
                    if (txtdatebegin.Text != "" && txtdateend.Text != "")
                    {
                        ...
                        {
                            startDateTime = startDateTime.ChangeTimes(Convert.ToInt32(txthrbegin.Text), Convert.ToInt32(txtminbegin.Text), 0, 0);
                            endDateTime = endDateTime.ChangeTimes(Convert.ToInt32(txthrend.Text), Convert.ToInt32(txtminend.Text), 0, 0);
                        }
                        else if (txthrbegin.Text == "" && txthrend.Text == "" && txtminbegin.Text == "" && txtminend.Text == "")
                        {
                            startDateTime = startDateTime.ChangeTimes(0, 0, 0, 0);
                            endDateTime = endDateTime.ChangeTimes(0, 0, 0, 0);
                        }
                    ...
                    { 
                        if (txtsddoc.Text != "" && txteddoc.Text != "")
                        {
                            ...
                            {
                                startDocDateTime = startDocDateTime.ChangeTimes(Convert.ToInt32(txtshdoc.Text), Convert.ToInt32(txtsmdoc.Text), 0, 0);
                                enddocDateTime = enddocDateTime.ChangeTimes(Convert.ToInt32(txtehdoc.Text), Convert.ToInt32(txtemdoc.Text), 0, 0);
                            }
                            else if (txtshdoc.Text == "" && txtehdoc.Text == "" && txtsmdoc.Text == "" && txtemdoc.Text == "")
                            {
                                startDocDateTime = startDocDateTime.ChangeTimes(0, 0, 0, 0);
                                enddocDateTime = enddocDateTime.ChangeTimes(0, 0, 0, 0);
                            } 
        ...
        private void bindingDataGrid(NewBISSvcRef.NewBISSvcClient client, ref decimal sumPrem, ref decimal sumSumm)
        {
            ...
            foreach (DataGridViewRow dr in dtgCallReport.Rows)
            {
                if (dr.DataBoundItem is NewBISSvcRef.APP_IN_PROCESS)
                {
                    ...
                    dr.Cells["clmpolicy"].Value = obj.POLICY;
                    dr.Cells["clmisis"].Value = obj.ISIS_STATUS;
                    dr.Cells["clmname"].Value = obj.CUSTNAME;
                    dr.Cells["clmsex"].Value = obj.SEX.ToString() == "M" ? "ช" : "ญ"; //obj.SEX; 
                    dr.Cells["clmbirthdt"].Value = String.Format("{0:dd/MM/yyyy}", obj.BIRTH_DT);
            ...
            txtsumapp.Text = listAppnoDistinct.Count().ToString(); 
        }
        ...
        private void printToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                if (dtgCallReport.RowCount > 0)
                {
                    ...
                    NewBISSvcRef.APP_IN_PROCESS[] appInProcFilterLockedItem = appInProcs.ToArray();

                    ReportForm.frmReportAppList repAppList = new ReportForm.frmReportAppList(appInProcFilterLockedItem, condition, userName, txtsumapp.Text, txtsumm.Text, txtpremium.Text);
                    repAppList.Show();
                }
                else
                {
                    MessageBox.Show("ไม่มีข้อมูลที่จะต้องพิมพ์");
                }
            }
            finally
            {
        ...
        private void FormApplicationViewer_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void txtdateappwait_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyValue == 13 && txtdateappwait.Text != "")
            {
                setFormatDate(e, txtdateappwait);
                txtdateappwait.Focus();
            }
        }
    ...
    public static class DateModuleViewer  //Extension Method for changedate ind datetime variable
    { 
        public static DateTime ChangeTimes(this DateTime dateTime, int hours, int minutes, int seconds, int milliseconds)
        {
            return new DateTime(
                dateTime.Year,
                dateTime.Month,
